...
type Message={title:string;body:string;confirm?:string;cancel?:string;resolve:(value:boolean)=>void};
export function useDialogs(){
  const [message,setMessage]=useState<Message|null>(null);
  const pending=useRef<((value:boolean)=>void)|null>(null);
  useEffect(()=>()=>{pending.current?.(false);},[]);
  function ask(title:string,body:string,confirm?:string,cancel?:string){pending.current?.(false);return new Promise<boolean>(resolve=>{pending.current=resolve;setMessage({title,body,confirm,cancel,resolve});});}
  function close(value:boolean){message?.resolve(value);pending.current=null;setMessage(null);}
  return {ask,dialog:message&&<Modal title={message.title} onClose={()=>close(false)}><p>{message.body}</p><div className="dialog-actions">{message.confirm&&<button onClick={()=>close(false)}>{message.cancel || '취소'}</button>}<button className="primary" onClick={()=>close(true)}>{message.confirm || '확인'}</button></div></Modal>};
}
