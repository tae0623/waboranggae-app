...
export function defaultCondition():Condition {return {
  departure:'',region:'순천',date:todayKorea(),endDate:todayKorea(),startTime:'10:00',endTime:'16:00',endTimeLimited:false,
...
export function DatePicker({value,onChange}:{value:string;onChange:(s:string)=>void}){
  ...
  return <><button className="field-button" aria-label="여행 날짜" onClick={()=>{setDraft(value);setMonth(value.slice(0,7));setOpen(true);}}><span>여행 날짜</span><strong>{value.replaceAll('-','. ')}</strong></button>
    {open&&<Modal title="여행 날짜" onClose={()=>setOpen(false)}><div className="calendar-head"><button aria-label="이전 달" onClick={()=>move(-1)}>‹</button><strong>{year}년 {mon}월</strong><button aria-label="다음 달" onClick={()=>move(1)}>›</button></div>
...
function DepartureSearch({condition,onChange}:{condition:Condition;onChange:(v:Partial<Condition>)=>void}){
  ...
  const mapUrl=preview?(getWebRuntime().mapBaseUrl||window.location.origin)+'/maps/embed#'+encodeURIComponent(JSON.stringify({origin:preview,places:[],routeSegments:[],conveniences:[]})):null;
  return <><label className="field-label" htmlFor="departure">출발지</label><div className="departure-field"><input id="departure" role="combobox" aria-controls="departure-results" aria-expanded={results.length>0} aria-autocomplete="list" aria-activedescendant={highlight>=0?'departure-option-'+highlight:undefined}
    ...
    {mapUrl?<iframe className="departure-map" title="검색한 출발지 지도" src={mapUrl}/>:<div className="departure-map placeholder">검색한 장소를 지도에서 확인하세요</div>}
    <p className="source-footer">카카오 검색 · 정확도순 · 지도 © Kakao</p>
  </>;
}
...
export function PlanWizard({initial,onClose,onComplete}:{initial?:Partial<Condition>;onClose:()=>void;onComplete:(c:Condition)=>void}){
  const [cond,setCond]=useState<Condition>(()=>({...defaultCondition(),...initial,endDate:initial?.date||todayKorea(),companion:'혼자',transitModes:['bus']}));
  const [step,setStep]=useState(1),[error,setError]=useState(''),[regionOpen,setRegionOpen]=useState(false);
  const body=useRef<HTMLDivElement>(null);
  function update(p:Partial<Condition>){setCond(old=>{const c={...old,...p};return {...c,endDate:c.date,meals:normalizeMeals(c)};});setError('');}
  ...
  return <div className="travel-wizard" role="dialog" aria-modal="true" aria-label="여행 조건">
    ...
    <div className="wizard-body" ref={body}><span className="step-label">STEP 0{step}</span><h1>{TITLES[step-1]}</h1><p className="wizard-subtitle">{SUBTITLES[step-1]}</p>
      ...
      {step===2&&<><button className="destination-card" onClick={()=>setRegionOpen(true)} aria-label="여행지 선택"><small>전라남도</small><strong>{cond.region||'여행지 선택'} <span>⌄</span></strong></button>
        <DatePicker value={cond.date} onChange={date=>update({date})}/><TimePicker label="현지 여행 시작" value={cond.startTime} onChange={startTime=>update({startTime})}/>
        <label className="option-row"><span>종료 시간 정하기<small>돌아갈 시간이 정해져 있을 때만 선택하세요.</small></span><input type="checkbox" checked={!!cond.endTimeLimited} onChange={e=>update({endTimeLimited:e.target.checked})}/></label>
