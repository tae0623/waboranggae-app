...
function contentTypesFor(preferences: TravelPreferences) {
  ...
  if (preferences.mealPreference !== 'none' || (preferences.meals?.length ?? 0) > 0) types.add(39);
  return [...types];
}
...
const HIGHLIGHT_TYPES = new Set([12, 14, 15, 25, 28, 38]);
const BLOCKED_HIGHLIGHT_NAME = /모텔|호텔|펜션|리조트|글램핑|캠핑장|민박|게스트하우스|유스호스텔/i;

function isHighlightAttraction(item: TourApiItem) {
  if (BLOCKED_HIGHLIGHT_NAME.test(item.title || '')) return false;
  const type = contentType(item);
  if (type === 32) return false;
  return HIGHLIGHT_TYPES.has(type) || type === 0;
}

function highlightFromItem(item: TourApiItem, city: string): TourHighlight | null {
  const name = item.title?.trim();
  if (!name) return null;
  const latitude = Number(item.mapy);
  const longitude = Number(item.mapx);
  return {
    id: item.contentid || `${city}-${name}`,
    name,
    city,
    category: classifyTourItemCategory(item),
    tags: classifyTourItemTags(item),
    address: item.addr1 || `${city} · 전라남도`,
    imageUrl: normalizeTourImageUrl(item.firstimage || item.firstimage2),
    latitude: Number.isFinite(latitude) ? latitude : undefined,
    longitude: Number.isFinite(longitude) ? longitude : undefined,
    contentTypeId: contentType(item) || undefined,
    eventStartDate: item.eventstartdate,
    eventEndDate: item.eventenddate,
  };
}

export async function fetchKeywordHighlights(keyword: string, limit = 8): Promise<TourHighlight[]> {
  if (!serviceKey || !keyword.trim()) return [];
  try {
    const { regionCode } = await resolveLegalArea('전라남도', '순천');
    const items = await requestItems('searchKeyword2', {
      numOfRows: '20',
      pageNo: '1',
      arrange: 'Q',
    ...
    return items
      ...
      .map((item) => {
        const haystack = `${item.lDongSignguNm || ''} ${item.addr1 || ''} ${item.title || ''}`;
        const city = JEONNAM_CITIES.find((name) => haystack.includes(name)) ?? '전남';
        return highlightFromItem(item, city);
      })
      .filter((item): item is TourHighlight => Boolean(item?.imageUrl))
      .slice(0, limit);
  } catch (error) {
    const message = error instanceof Error ? error.message : '알 수 없는 오류';
    console.warn(`[waboranggae] TourAPI 키워드 조회 실패(${keyword}): ${message}`);
    return [];
  }
}

export async function fetchCityHighlights(city: string, limit = 8): Promise<TourHighlight[]> {
  if (!serviceKey) return [];
  try {
    const { regionCode, cityCode } = await resolveLegalArea('전라남도', city);
    ...
    return items
      .filter(isHighlightAttraction)
      .map((item) => highlightFromItem(item, city))
      .filter((item): item is TourHighlight => Boolean(item))
      .sort((a, b) => Number(Boolean(b.imageUrl)) - Number(Boolean(a.imageUrl)))
      .slice(0, limit);
  } catch (error) {
    const message = error instanceof Error ? error.message : '알 수 없는 오류';
    console.warn(`[waboranggae] TourAPI 핫플레이스 조회 실패(${city}): ${message}`);
    return [];
  }
}
...
export async function fetchOngoingFestivals(limit = 4): Promise<TourHighlight[]> {
  if (!serviceKey) return [];
  try {
    const regions = await requestItems('ldongCode2', {
      numOfRows: '50',
    ...
    return items
      ...
      .map((item) => {
        ...
        return matched ? highlightFromItem(item, matched) : null;
      })
      .filter((item): item is TourHighlight => Boolean(item))
      .slice(0, limit);
  } catch (error) {
    const message = error instanceof Error ? error.message : '알 수 없는 오류';
    console.warn(`[waboranggae] TourAPI 축제 조회 실패: ${message}`);
    return [];
  }
}
...
function textFrom(item: Record<string, unknown> | null | undefined, keys: string[]) {
  if (!item) return '';
...
function homepageFrom(value: string) {
  const href = value.match(/href=["']([^"']+)/i)?.[1]?.trim();
...
export async function fetchTourPlaceDetail(
  ...
  const type = contentTypeId ? String(contentTypeId) : '';
  try {
    ...
    const common = (commonItems[0] || null) as Record<string, unknown> | null;
    const intro = (introItems[0] || null) as Record<string, unknown> | null;
    if (!common && !intro) return null;
    const homepage = homepageFrom(textFrom(common, ['homepage']) || textFrom(intro, ['eventhomepage']));
    return {
      overview: textFrom(common, ['overview']),
      tel: textFrom(intro, ['sponsor1tel', 'infocenter', 'infocenterculture', 'infocenterleports', 'infocenterfood'])
        || textFrom(common, ['tel']),
      homepage,
      fee: textFrom(intro, ['usefestival', 'usefee', 'usefeeleports', 'treatmenu']),
      hours: textFrom(intro, ['usetimefestival', 'playtime', 'usetime', 'usetimeculture', 'usetimeleports', 'opentimefood']),
      eventPlace: textFrom(intro, ['eventplace', 'placeinfo']),
      startDate: textFrom(intro, ['eventstartdate']) || textFrom(common, ['eventstartdate']),
      endDate: textFrom(intro, ['eventenddate']) || textFrom(common, ['eventenddate']),
      sponsor: textFrom(intro, ['sponsor1', 'sponsor2']),
      ageLimit: textFrom(intro, ['agelimit']),
      spendTime: textFrom(intro, ['spendtimefestival']),
      program: textFrom(intro, ['program', 'subevent']),
      restDate: textFrom(intro, ['restdate', 'restdateculture', 'restdateleports', 'restdatesports']),
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : '알 수 없는 오류';
    console.warn(`[waboranggae] TourAPI 상세 조회 실패(${contentId}): ${message}`);
    return null;
  }
}
