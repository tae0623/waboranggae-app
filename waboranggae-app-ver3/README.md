...
API 키는 모바일 앱에 넣지 않습니다. `DATA_GO_KR_KEY`와 `TMAP_TRANSIT_API_KEY`는 Express 서버의 `.env`에서만 읽습니다.
...
DATA_GO_KR_KEY=공공데이터포털에서_발급받은_서비스키
TMAP_TRANSIT_API_KEY=TMAP에서_발급받은_앱키
TMAP_LIVE_COURSE_LIMIT=1
TMAP_DAILY_REQUEST_LIMIT=10
```

`DATA_GO_KR_KEY`는 `https://apis.data.go.kr/`로 시작하는 TourAPI·버스정류소·버스노선·공영 물품보관함 요청에 공통으로 사용합니다. 각 API의 활용신청은 공공데이터포털에서 별도로 승인돼 있어야 합니다. 키가 비어 있으면 TourAPI는 시연 코스로 전환되고 물품보관함은 표시하지 않습니다.
`TMAP_TRANSIT_API_KEY`가 비어 있으면 선택한 역·터미널·직접 입력 거점의 실좌표는 유지하되, 구간은 점선과 `좌표 기반 예상`으로 명확히 표시합니다. 키를 넣으면 첫 출발 구간을 포함한 도보·대중교통 시간과 경로선이 TMAP 결과로 전환됩니다. 무료 호출량 보호를 위해 기본값은 1순위 코스만 TMAP을 사용하고 하루 10회에서 외부 호출을 멈춥니다. 나머지 추천 코스도 지도 탭에서 선택해 볼 수 있으며 예상 경로로 표시됩니다.
...
3. 서버가 TourAPI 장소를 가져오고 시간·식사·거리 규칙으로 최대 3개 일정을 빠르게 구성합니다.
4. 서버가 출발 거점, 식사 시간, 식사 후 카페, 연속 음식점·카페, 장소 중복, 선택 시간 충족을 검사합니다. 2~8시간 선택은 최대 30분 오차만 허용합니다. `OLLAMA_COURSE_PLANNER_ENABLED=true`는 LLM 일정 제안을 비교하는 실험용 옵션입니다.
5. 검증된 코스를 대중교통 접근성·도보 부담·주변 연계성으로 각각 0~100점 표준화하고, 합계 100% 가중치로 점수화합니다.
...
역이 없는 지역은 `터미널` 또는 `직접 입력`을 선택해 버스정류장, 여객터미널, 선착장, 숙소, 관광지 등을 출발 거점으로 지정합니다. 서버는 이 이름을 지오코딩해 코스 시간표와 지도의 동일한 출발 마커에 사용합니다.

### 뚜벅이 점수 가중치

| 여행 스타일 | 대중교통 접근성 | 도보 부담도 | 주변 관광 연계성 |
| --- | ---: | ---: | ---: |
| 여유롭게 | 35% | 50% | 15% |
| 균형 있게 | 40% | 30% | 30% |
| 알차게 | 35% | 20% | 45% |

도보 부담은 여행시간당 도보 분, 주변 연계성은 구간당 평균 이동거리를 기준으로 표준화합니다. 대중교통 접근성은 장소별 최근접 정류장 거리 70%와 실제 경유 노선 공급 30%를 결합하며, 노선 API가 승인된 경우 대표 노선의 평일 배차간격도 노선 공급 점수에 반영합니다. 지역이나 관심사 보너스와 물품보관함 유무는 최종 뚜벅이 점수에 섞지 않습니다.
...
UI에서 사용자가 확인한 `preferences`를 전송합니다. 자연어 보조 기능을 썼다면 `/api/analyze` 결과를 UI에서 수정한 최종값이 전달됩니다. 서버가 TourAPI 관광지를 내부 `Course` 모델로 변환하고, 시간 규칙과 결정적 검증을 거친 뒤 코스 점수를 계산합니다. 동일 조건 결과는 15분간 캐시합니다. 키가 없거나 조회에 실패하면 시연 코스를 반환합니다. LLM 코스 계획 실험은 `OLLAMA_COURSE_PLANNER_ENABLED=true`일 때만 추천 응답에 포함됩니다.
...
- 지도 마커와 코스 상세의 관광 이미지는 TourAPI의 대표 이미지 URL을 사용하며, 이미지가 없는 장소는 텍스트 마커로 유지합니다.
...
- TAGO 버스정류소는 최근접 정류장 거리와 경유 노선 수를 계산하고, TAGO 버스노선은 대표 노선의 배차정보를 보강합니다. 실제 구간 길찾기는 TMAP을 사용하므로 정적 노선정보를 경로처럼 추정하지 않습니다.
- 공영 물품보관함은 공공데이터에 실제 위치가 있고 코스 반경 5km 안일 때만 상세·지도에 `실데이터`로 표시하며, 데모 위치를 만들지 않습니다.
...
`pnpm llm:benchmark`는 자연어 조건 4개를 순서대로 분석해 Ollama 사용 여부, 필드 정답률, 첫 응답·중앙값·p95 시간을 출력합니다. 배포 서버 비교는 PowerShell에서 `$env:API_BASE_URL='https://배포주소'; pnpm llm:benchmark`로 실행합니다. 첫 요청은 모델 적재 시간이 포함될 수 있어 이후 요청과 함께 비교하세요.
