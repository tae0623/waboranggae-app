...
async function routeBetween(
  ...
  const live = await fetchTmapTransitRoute(from, to, searchDateTime(preferences));
...
function buildSchedule(
  ...
  const scheduled = places.map((place, index) => {
    ...
    const routeText = segment?.source === 'tmap-transit'
      ? `TMAP ${segment.modeLabel} ${segment.totalMinutes}분${segment.walkMinutes ? ` (도보 ${segment.walkMinutes}분 포함)` : ''}`
      : `좌표 추정 ${segment?.modeLabel ?? '이동'} ${segment?.totalMinutes ?? 0}분`;
    return {
      ...place,
      arrival,
      stayMinutes,
...
async function routeCourse(
  ...
  const segments = await Promise.all(
    routablePlaces.map((_, index) => routeBetween(points[index]!, points[index + 1]!, preferences)),
  );
  ...
  const routingNote = source === 'tmap-transit'
    ...
    : source === 'mixed'
      ? '일부 구간 TMAP 실제 경로 반영'
      : `출발 거점 실좌표 반영 · ${isTmapTransitConfigured() ? '길찾기 실패 구간 좌표 추정' : 'TMAP 키 미설정으로 이동시간 추정'}`;
  ...
  return {
    ...
    metrics: {
      ...course.metrics,
      walkingEase: Math.round(Math.max(40, 100 - walkMinutes * 0.75)),
    },
...
export async function attachRoutingToCourses(
  ...
  return Promise.all(courses.map((course) => routeCourse(preferences, course, origin)));
}
