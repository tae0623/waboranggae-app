...
const val NOTICE_VERSION="2026-09-15-consent-v3"
...
class AccountViewModel(private val repository:TravelRepository):ViewModel() {
    ...
    fun loginScreen() { mutable.update{it.copy(showLogin=true,message=null)};refreshProviders() }
    fun browserFailed() {
        action?.cancel()
    ...
    private fun task(block:suspend ()->Unit) {
        ...
        action=viewModelScope.launch {
            try{block()}catch(e:Exception){if(e is CancellationException)throw e;mutable.update{it.copy(message=when((e as? ApiFailure)?.status){401->"로그인 정보가 올바르지 않거나 만료되었습니다. 다시 로그인해 주세요.";409->"이미 등록되었거나 동시에 변경된 정보입니다. 상태를 확인해 주세요.";else->e.message?:"요청에 실패했습니다."})}}
    ...
    fun login(email:String,password:String,name:String,signup:Boolean,consent:Boolean) {
        ...
        task {
            val body=buildJsonObject{put("email",email.trim());put("password",password);if(signup){put("displayName",name.trim());put("privacyConsent",true);put("consentVersion",NOTICE_VERSION)}}
    ...
    fun agreePrivacy(consent:Boolean) {
        ...
        task {
            ...
            val body=buildJsonObject{put("privacyConsent",true);put("consentVersion",NOTICE_VERSION);if(pending!=null){put("flowId",pending.text("flowId"));put("pollSecret",pending.text("pollSecret"))}}
            if(pending!=null){
