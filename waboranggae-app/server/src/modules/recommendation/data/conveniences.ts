...
const serviceKey = (process.env.TOUR_API_KEY || process.env.DATA_GO_KR_KEY || process.env.LOCKER_API_KEY || '').trim();
...
export async function fetchLockersNear(
  ...
  if (!serviceKey) return demoLockers(city, center);
  ...
  try {
    ...
    const spots = rows
      ...
      .filter((item): item is ConvenienceSpot => Boolean(item))
...
export async function fetchBikesNear(
  ...
  if (!serviceKey) return demoBikes(city, center);
