import { RouteCoordinate, RouteSegment, TransitStep } from '../../../../../src/types/travel';
import { formatTransitInstruction, vehicleFromTmap } from '../transit-instruction';
...
type TmapLeg = {
  ...
  lane?: Array<{ route?: string; routeId?: string }>;
};
...
function routeNameFromLeg(leg: TmapLeg) {
  return String(leg.route || leg.Route || leg.routeNm || leg.lane?.[0]?.route || '').trim();
}

export function stepsFromTmapLegs(legs: TmapLeg[]): TransitStep[] {
  return legs.flatMap((leg) => {
    const minutes = Math.max(0, Math.ceil(Number(leg.sectionTime || 0) / 60));
    if (!minutes && leg.mode === 'WALK') return [];
    const vehicle = vehicleFromTmap(leg.mode, routeNameFromLeg(leg));
    const fromStop = vehicle.mode === 'walk' ? undefined : (leg.start?.name || '').replace(/\s+/g, ' ').trim() || undefined;
    const toStop = vehicle.mode === 'walk' ? undefined : (leg.end?.name || '').replace(/\s+/g, ' ').trim() || undefined;
    return [{
      ...vehicle,
      minutes: Math.max(vehicle.mode === 'walk' ? 0 : 1, minutes),
      fromStop,
      toStop,
    }];
...
export function parseTmapTransitResponse(
  ...
  const steps = stepsFromTmapLegs(legs);
  ...
  return {
    ...
    instruction: formatTransitInstruction(steps),
    steps,
    source: 'tmap-transit',
    geometry,
  };
