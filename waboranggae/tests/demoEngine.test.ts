...
describe('walkability ranking', () => {
  it('ranks the requested city first and returns grounded evidence', () => {
    ...
    expect(courses[0]?.preferenceScore).toBeGreaterThan(60);
    expect(courses[0]?.walkingScore).toBeGreaterThan(40);
    expect(courses[0]?.reason.evidence.join(' ')).toContain('취향 적합도');
    expect(courses[0]?.reason.evidence.join(' ')).toContain('뚜벅이 적합도');
    expect(courses[0]?.reason.summary).toMatch(/총 도보 \d+분/);
  });
});
