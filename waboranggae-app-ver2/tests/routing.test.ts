...
describe('directions integration', () => {
        ...
        plan: {
          itineraries: [{
            ...
            legs: [
              ...
              { mode: 'BUS', sectionTime: 960, passShape: { linestring: '127.493,34.946 127.5095,34.9289' } },
            ],
          }],
        },
      },
  ...
  it('renders the selected terminal as a separate origin marker and starts the route there', () => {
    const course = {
      ...
      metrics: { transitAccess: 90, walkingEase: 90, nearbyLinks: 80, convenience: 70 },
      places: [{
        id: 'garden', name: '순천만국가정원', category: 'nature', address: '순천시', stayMinutes: 90,
        arrival: '10:21', moveLabel: 'TMAP 버스 21분', description: '', tags: ['nature'], mapPoint: { x: 100, y: 100 },
        latitude: to.latitude, longitude: to.longitude,
      }],
      conveniences: [], fitScore: 95, scoreBreakdown: { transitAccess: 90, walkingEase: 90, nearbyLinks: 80, convenience: 70 },
      ...
      routeSegments: [{
        ...
        transitMinutes: 16, modeLabel: '도보·버스', source: 'tmap-transit', geometry: [from, to],
      }],
