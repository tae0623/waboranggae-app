...
function requestedKinds(mealPreference: MealPreference): MealWindow['kind'][] {
...
export function mealWindowsFor(preferences: TravelPreferences): MealWindow[] {
  if (preferences.mealPreference === 'none') return [];
  const start = parseClock(preferences.startTime);
  const end = start + preferences.durationHours * 60;
  const kinds = preferences.mealPreference === 'auto'
    ? (['lunch', 'dinner'] as const)
    : requestedKinds(preferences.mealPreference);
  return kinds
    .map((kind) => MEAL_WINDOWS[kind])
    .filter((window) => overlapsTrip(window, start, end));
}
...
export function collectPlanningCandidates(courses: Course[]) {
...
function schedulePlaces(preferences: TravelPreferences, sourcePlaces: Place[]) {
  ...
  const targetEnd = parseClock(preferences.startTime) + preferences.durationHours * 60;
...
export function validateScheduledPlaces(preferences: TravelPreferences, places: Place[]) {
  ...
  const end = last ? parseClock(last.arrival) + last.stayMinutes : start;
  if (end > start + preferences.durationHours * 60 + 20) {
    violations.push('전체 일정이 선택한 여행 시간을 초과했습니다.');
  }
  if (end < start + minimumCourseDurationMinutes(preferences.durationHours)) {
  ...
  const maxTotalKm = preferences.pace === 'easy' || preferences.lowMobility
    ? Math.max(8, preferences.durationHours * 2)
    : preferences.pace === 'full'
      ? Math.max(18, preferences.durationHours * 4)
      : Math.max(12, preferences.durationHours * 3);
...
function buildCourse(
  ...
  const metrics: WalkabilityMetrics = {
    ...
    walkingEase: Math.round(Math.max(45, 96 - walkingDistance * 12)),
    nearbyLinks: Math.round(Math.max(55, 95 - totalDistance * 2)),
    convenience: 60,
  };
  const start = parseClock(preferences.startTime);
  ...
  return {
    ...
    validationNotes: [
      ...
      '식사 후 카페 순서 검증',
      '동일 식음 장소 연속 배치 방지',
      `선택 시간 ${preferences.durationHours}시간 충족 검증`,
    ],
  } satisfies Course;
}

export function desiredStopCount(durationHours: number) {
  return Math.max(2, Math.min(7, Math.round(durationHours / 1.5)));
}

function mealSlotIndexes(preferences: TravelPreferences, stopCount: number) {
  const start = parseClock(preferences.startTime);
  const interval = Math.max(70, Math.min(95, preferences.durationHours * 60 / Math.max(stopCount, 1)));
...
function candidateScore(candidate: Place, previous: Place | undefined, preferences: TravelPreferences, previousCategory: PlaceCategory | undefined) {
  const interestBonus = candidate.tags.filter((tag) => preferences.interests.includes(tag)).length * 8;
  const distancePenalty = previous ? segmentDistance(previous, candidate) * 2 : 0;
  ...
  return interestBonus + imageBonus - distancePenalty - repeatPenalty - refreshmentPenalty;
}
...
function buildRuleOutline(preferences: TravelPreferences, candidates: Place[], routeIndex: number) {
  const stopCount = Math.min(desiredStopCount(preferences.durationHours), candidates.length);
...
export function buildRulePlannedCourses(preferences: TravelPreferences, candidates: Place[]) {
  ...
  for (let index = 0; index < 3; index += 1) {
    const places = buildRuleOutline(preferences, candidates, index);
...
function repairOutlinePlaces(preferences: TravelPreferences, selected: Place[], candidates: Place[]) {
  ...
  const totalCount = Math.max(2, Math.min(
    desiredStopCount(preferences.durationHours),
...
export function mergePlannedCourses(
  ...
  const candidates = collectPlanningCandidates(sourceCourses);
  ...
  const ruleCourses = sourceCourses.flatMap((sourceCourse) =>
    buildRulePlannedCourses(preferences, collectPlanningCandidates([sourceCourse])),
  );
  const merged: Course[] = [];
